 ...
 */
package com.ips.upmp.nohttp;

import android.app.Dialog;
import android.content.DialogInterface;

import com.ips.upmp.R;
import com.ips.upmp.activity.BaseActivity;
import com.ips.upmp.dialog.IPSDialog;
import com.ips.upmp.dialog.WaitDialog;
import com.ips.upmp.util.MakeToast;
...
public class HttpResponseListener<T> implements OnResponseListener<T> {
    ...
    public HttpResponseListener(BaseActivity activity,Request<?> request, HttpListener<T> httpCallback, boolean canCancel, boolean isLoading) {
        ...
        if (activity != null && isLoading) {
            ...
            mWaitDialog = IPSDialog.show(activity, "");
    ...
    public void onFailed(int what, String url, Object tag, Exception exception, int responseCode, long networkMillis) {
        if (exception instanceof NetworkError) {// 网络不好
            MakeToast.show(mActivity, R.string.error_please_check_network);
        } else if (exception instanceof TimeoutError) {// 请求超时
            MakeToast.show(mActivity, R.string.error_timeout);
        } else if (exception instanceof UnKnownHostError) {// 找不到服务器
            MakeToast.show(mActivity, R.string.error_not_found_server);
        } else if (exception instanceof URLError) {// URL是错的
            MakeToast.show(mActivity, R.string.error_url_error);
        } else if (exception instanceof NotFoundCacheError) {
            // 这个异常只会在仅仅查找缓存时没有找到缓存时返回
            MakeToast.show(mActivity, R.string.error_not_found_cache);
        } else if (exception instanceof ProtocolException) {
            MakeToast.show(mActivity, R.string.error_system_unsupport_method);
        } else if (exception instanceof ParseError) {
            MakeToast.show(mActivity, R.string.error_parse_data_error);
        } else {
            MakeToast.show(mActivity, R.string.error_unknow);
        }
